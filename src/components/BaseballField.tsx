...
interface PlayerCardProps {
  ...
  function PlayerCard({ name, position, points,  jersey, isCaptain, isStarter }: PlayerCardProps) {
    return (
      <div className="relative w-32 text-center">
        <div className="flex flex-col items-center">
          ...
          <div className="z-10 flex items-center justify-center gap-2 text-xs mt-0.5">
            <div className="px-1.5 py-0.5 rounded bg-[#FDFBF7] text-[#8B7355] font-medium">+{points}</div>
          </div>
        </div>
      </div>
    )
...
async function generatePlayerPoints(players: any[]) {
  ...
  const prompt = `Generate fantasy baseball points for these players: ${players.map(p => p.player_first_name).join(', ')}. 
    Points should be between 0 and 100. Return as JSON with playerPoints array containing objects with name and points.`;
  ...
  try {
    ...
    const totalScore = response.playerPoints.reduce((sum: number, player: any) => sum + player.points, 0);
    
    // Update points in the database and save total score
    for (const playerPoint of response.playerPoints) {
      const player = players.find(p => p.player_first_name === playerPoint.name);
      if (player) {
...
export default function BaseballField() {
    ...
    async function fetchPlayers() {
      ...
      try {
        ...
        const playersWithPositions = data.map(player => ({
          ...
          points: pointsData.find(p => p.name === player.player_first_name)?.points || 0
        }));
  ...
  const assignPlayerToPosition = async (playerId: string, position: string) => {
    try {
      ...
      const currentPositionPlayer = players.find(p => p.assigned_position === position)
  ...
  const renderPlayerCard = (position: string, isStarter?: boolean, isCaptain?: boolean) => {
    ...
    return (
      <div onClick={() => handlePositionClick(position)}>
        <PlayerCard
          ...
          jersey={player ? player.player_image : "/placeholder.svg?height=64&width=64"}
          isStarter={isStarter}
          isCaptain={isCaptain}
        />
      </div>
  ...
  return (
    <div className="w-full max-w-3xl mx-auto">
      <div className="relative aspect-[4/3] bg-[#2F6B25] rounded-lg overflow-hidden">
        ...
        <svg viewBox="0 0 400 300" className="absolute inset-0 w-full h-full" style={{ filter: "brightness(1.2)" }}>
          <path d="M200 250 L300 150 L200 50 L100 150 Z" fill="none" stroke="#8B4513" strokeWidth="4" />
          <circle cx="200" cy="150" r="50" fill="#C19A6B" />
          <path d="M200 250 L200 150" stroke="#8B4513" strokeWidth="4" strokeDasharray="4" />
          <path d="M100 150 L300 150" stroke="#8B4513" strokeWidth="4" strokeDasharray="4" />
        </svg>
      ...
      {showModal && selectedPosition && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
          <div className="bg-white p-4 rounded-lg max-w-md w-full">
            <h3 className="text-lg font-bold mb-4">Select Player for {selectedPosition}</h3>
            <div className="space-y-2">
              {getEligiblePlayers(selectedPosition).map((player) => (
                <button
                  key={player.player_id}
                  className="w-full p-2 text-left hover:bg-gray-100 rounded"
                  onClick={() => assignPlayerToPosition(player.player_id, selectedPosition)}
                >
                  {player.player_first_name} ({player.player_position})
                </button>
              ))}
            </div>
            <button
